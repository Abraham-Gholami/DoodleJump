...
public class File_Manager : MonoBehaviour {
    ...
    {
        if(PlayerPrefs.HasKey("Info"))//File.Exists(File_Path))
        {
            string Str_Data = PlayerPrefs.GetString("Info");// File.ReadAllLines(File_Path);
            ...
            {*/
               ...
               var data = Str_Data.Split('|');
                ...
                if (data.Length != 3) // File corrupted
                {
                    Data_Manager.Set_PlayerName(string.Empty);
                    Data_Manager.Set_PlayerHash(string.Empty);
                    Data_Manager.Set_HighScore(0);
                ...
                {
                    Data_Manager.Set_PlayerName(data[0]);
                    Data_Manager.Set_PlayerHash(data[1]);
                    Data_Manager.Set_HighScore(System.Convert.ToInt32(data[2]));
                }
            //}
        }
    }
    ...
    public static void Save_Info()
    {
        ...
        PlayerPrefs.SetString("Info", Str_Data);
