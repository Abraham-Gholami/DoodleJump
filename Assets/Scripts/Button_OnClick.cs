...
public class Button_OnClick : MonoBehaviour {
    ...
    {
        GameObject mainMenuCanvas = GameObject.Find("GameController");

        // Active highscore menu
        mainMenuCanvas.GetComponent<Submit_Score>().HighScore_Canvas.SetActive(true);
        ...
        for (int i = 1; i < 14; i++)
            ...
            // Active loading
            mainMenuCanvas.GetComponent<Submit_Score>().HighScore_Canvas.transform.GetChild(5).gameObject.SetActive(true);

        // Show scores
        mainMenuCanvas.GetComponent<Submit_Score>().Show_TopTen();
    }
    ...
    public void SubmitConfirm_OnClick()
    {
        ...
        {
            GameObject.Find("GameController").GetComponent<Submit_Score>().Post_Score(Name, High_Score);
        }
    }
    ...
    public void Share_OnClick()
    {
        ...
        // Share score
        GameObject.Find("GameController").GetComponent<Share_Score>().Share();
    ...
    {
        var gameOverCanvas = GameObject.FindObjectsByType<GameOverCanvas>(FindObjectsSortMode.None)[0];

        if (State)
        {
            gameOverCanvas.DisplayGameOver(); //Data_Manager.Get_PlayerName());
        }
        else
        {
    ...
    public static void Set_SubmitScoreMenu(bool State)
    {
        ...
        for (int i = 12; i < 17; i++)
            gameOverCanvas.transform.GetChild(i).gameObject.SetActive(State);

        // Set player name (if exist) and Input field set focus
        gameOverCanvas.transform.GetChild(14).GetComponent<InputField>().text = Data_Manager.Get_PlayerName();
        gameOverCanvas.transform.GetChild(14).GetComponent<InputField>().Select();
    }
    ...
    public static void Set_TopTenMenu(bool State)
    {
        ...
        GameObject HighScore_Canvas = GameObject.Find("GameController").GetComponent<Submit_Score>().HighScore_Canvas;
    ...
    public void OnQuitButton_OnClick()
    {
        Application.Quit();
    }
}
